		...
		if (this.rolling()) {
			if (this.tickCount % 5 == 0 && this.horizontalCollision && net.minecraftforge.event.ForgeEventFactory.getMobGriefingEvent(this.level, this)) {
	            ...
	            for(BlockPos blockpos : BlockPos.betweenClosed(Mth.floor(aabb.minX), Mth.floor(aabb.minY), Mth.floor(aabb.minZ), Mth.floor(aabb.maxX), Mth.floor(aabb.maxY), Mth.floor(aabb.maxZ))) {
	               ...
	               if (!blockstate.is(TagInit.Blocks.UNBREAKABLE) && blockstate.getDestroySpeed(this.level, blockpos) <= 10 && blockstate.getDestroySpeed(this.level, blockpos) >= 0) { // for explosion resistance, replace with: blockstate.getBlock().getExplosionResistance()
